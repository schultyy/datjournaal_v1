...
//
  ...
  const Channel = exports.Channel = (function () {
    function Channel(topic, params, socket) {
      ...
      this.pushBuffer = [];
      this.rejoinTimer = new Timer(() => _this2.rejoinUntilConnected(), this.socket.reconnectAfterMs);
      this.joinPush.receive('ok', () => {
        _this2.state = CHANNEL_STATES.joined;
        _this2.rejoinTimer.reset();
        _this2.pushBuffer.forEach(pushEvent => pushEvent.send());
        _this2.pushBuffer = [];
      });
      this.onClose(() => {
        _this2.rejoinTimer.reset();
    ...
    _createClass(Channel, [{
      ...
      key: 'join',
      value: function join() {
        const timeout = arguments.length <= 0 || arguments[0] === undefined ? this.timeout : arguments[0];
    ...
    value: function push(event, payload) {
      const timeout = arguments.length <= 2 || arguments[2] === undefined ? this.timeout : arguments[2];
    ...
    value: function trigger(event, payload, ref) {
      ...
      const error = CHANNEL_EVENTS.error;
      const leave = CHANNEL_EVENTS.leave;
      const join = CHANNEL_EVENTS.join;
  ...
  //
    ...
    function Socket(endPoint) {
      ...
      this.reconnectTimer = new Timer(() => {
        _this4.disconnect(() => _this4.connect());
    ...
    _createClass(Socket, [{
      ...
      value: function endPointURL() {
        const uri = Ajax.appendParams(Ajax.appendParams(this.endPoint, this.params), { vsn: VSN });
        if (uri.charAt(0) !== '/') {
          return uri;
        }
        if (uri.charAt(1) === '/') {
    ...
    value: function onConnOpen() {
      ...
      if (!this.conn.skipHeartbeat) {
        ...
        this.heartbeatTimer = setInterval(() => _this6.sendHeartbeat(), this.heartbeatIntervalMs);
      }
    ...
    value: function channel(topic) {
      const chanParams = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];
    ...
    _createClass(LongPoll, [{
      ...
      value: function poll() {
        ...
        Ajax.request('GET', this.endpointURL(), 'application/json', null, this.timeout, this.ontimeout.bind(this), (resp) => {
          ...
          switch (status) {
            case 200:
              messages.forEach(msg => _this8.onmessage({ data: JSON.stringify(msg) }));
              _this8.poll();
              break;
            case 204:
              _this8.poll();
  ...
  var Ajax = exports.Ajax = (function () {
    ...
    _createClass(Ajax, null, [{
      ...
      value: function xdomainRequest(req, method, endPoint, body, timeout, ontimeout, callback) {
        ...
        req.onload = function () {
          const response = _this10.parseJSON(req.responseText);
      ...
      value: function xhrRequest(req, method, endPoint, accept, body, timeout, ontimeout, callback) {
        ...
        req.onreadystatechange = function () {
          if (req.readyState === _this11.states.complete && callback) {
            const response = _this11.parseJSON(req.responseText);
            callback(response);
          }
        };
        if (ontimeout) {
  ...
  const Presence = exports.Presence = {
    ...
    syncDiff: function syncDiff(currentState, _ref2, onJoin, onLeave) {
      const joins = _ref2.joins;
      const leaves = _ref2.leaves;
